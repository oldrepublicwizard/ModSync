...
dotnet test src/ModSync.Tests/ModSync.Tests.csproj --filter "FullyQualifiedName=ModSync.Tests.MainWindowHeadlessTests.LandingPage_Shows_When_NoInstructionsLoaded" --configuration Debug
...
- **June 2026 landing PR queue:** see `AGENTS.md` (merge #133 → #130 → #134 → #135; #136–#139 + plan `096` branch independent on `master`; #138 superseded by plan `096`; superseded #120–#132 closed).
...
- `src/ModSync.Core` is the main runtime. It owns the instruction model (`Instruction`, `ModComponent`), dependency resolution, path handling, validation, install orchestration, checkpointing, download services, and HoloPatcher/Python integration.
- Instruction files are loaded through `src/ModSync.Core/Services/FileLoadingService.cs` and `src/ModSync.Core/Services/ModComponentSerializationService.cs`. The loader auto-detects TOML, Markdown, YAML, and JSON, then resolves component dependency order before the GUI works with the result.
- Dry-run validation and pre-install analysis are built around `src/ModSync.Core/Services/FileSystem/VirtualFileSystemProvider.cs`. It simulates file mutations against current disk state, and that virtual path is the expected flow for validation-style work.
- Real installs route through `src/ModSync.Core/Services/InstallationService.cs`, which also bootstraps the embedded Python environment used by the patching stack.
- `src/ModSync.GUI` is the Avalonia desktop shell. `MainWindow` composes many focused GUI services under `src/ModSync.GUI/Services`, while the install wizard lives under `src/ModSync.GUI/Dialogs/WizardPages`.
...
- `src/ModSync.Tests` is the only automated test project. It covers both core logic and Avalonia headless UI flows.
...
- All automated tests belong in `src/ModSync.Tests`; do not add more test projects. Tests that take more than 2 minutes should use the `LongRunning` suffix and are excluded from standard runs.
