...
Release Please generic extra-file updater preserved the legacy `a1` pre-release suffix when bumping from `2.0.0a1`. Users see `ModSync v2.1.0a1` in the window title while the shipped release is 2.1.0.
...
**Files:** `src/ModSync.Core/MainConfig.cs`

Change `"2.1.0a1"` → `"2.1.0"`; keep `// x-release-please-version` marker.

### U2. Add manifest parity test

**Files:** `src/ModSync.Tests/ReleaseVersionAlignmentTests.cs`
...
- `dotnet test src/ModSync.Tests/ModSync.Tests.csproj --filter "FullyQualifiedName~ReleaseVersionAlignment"`
