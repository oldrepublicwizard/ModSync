...
**Headless tests:** `dotnet test src/ModSync.Tests/ModSync.Tests.csproj --filter SettingsService` (plan `077`); `--filter StepProgressUiHelper` (plan `096`); `--filter StepNavigationService` (plan `093`).
