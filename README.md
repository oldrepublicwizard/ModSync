...
ModSync is a multi-mod installer for KOTOR games that makes it easier to install and manage mods.
...
This is why I created ModSync - an installer creator in C# to automate and simplify the entire process.
...
- All the compatibility steps are handled internally by ModSync and the default instructions files provided here. An end user simply can select the mods they want to install in the left list, and any customizations if they like, and simply press 'Install All' to have everything installed automatically.
...
- Create instructions files with complex dependency structures for multiple mods, and have end users install everything exactly according to the instructions created. No more manually copying/deleting files: ModSync handles all of that for your end user.
...
You'll need to click 'set directories' and browse to your game install (`<<gameDirectory>>`) and your mod folder (`<<modDirectory>>`).
...
- A: Yes! [Please submit your instructions here.](https://github.com/th3w1zard1/ModSync/discussions/categories/share-your-instructions) As long as your mod is SFW, I'll most likely add it to the list.
- Q: A mod in the instructions file is out of date, where do I report this?
- A: [Please notify me that mod has been updated here.](https://github.com/th3w1zard1/ModSync/discussions/categories/mod-updates)

## Supported Platforms

ModSync is a cross-platform 32-bit and 64-bit .NET application. It is compatible with the following operating systems:
...
All you need to do is build `src/ModSync.GUI/ModSync.csproj`. This builds the app into `src/ModSync.GUI/bin`. From the repo root, use `dotnet build ModSync.sln --configuration Debug`, or run `dotnet build` / `dotnet run --framework net9.0` inside `src/ModSync.GUI`.
...
Created HoloPatcher and the PyKotor library that ModSync uses to patch mods. These projects are the main reason ModSync can be supported on Mac/Linux. While the PyKotor/HoloPatcher projects have had some issues, this guy was more or less available for comment if I had questions on how I could fix any remaining problems myself.
...
ModSync is licensed under the Business Source License 1.1 (BSL 1.1).
