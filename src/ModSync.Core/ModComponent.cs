...
namespace ModSync.Core
{
    public class ModComponent : INotifyPropertyChanged
    {
            ...
            [NotNull] Services.FileSystem.IFileSystemProvider fileSystemProvider)
        {
            ...
            if (!NetFrameworkCompatibility.Contains(destRaw, "<<gameDirectory>>", StringComparison.OrdinalIgnoreCase)
                && !NetFrameworkCompatibility.Contains(destRaw, "<<kotorDirectory>>", StringComparison.OrdinalIgnoreCase))
            {
                return false;
            }
