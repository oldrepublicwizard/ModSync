    ...
    public static class DryRunValidator
    {
            ...
            [CanBeNull] ModComponent currentComponent)
        {
            ...
            try
            {
                ...
                if (!path.StartsWith("<<modDirectory>>", StringComparison.Ordinal) && !path.StartsWith("<<kotorDirectory>>", StringComparison.Ordinal) && !path.StartsWith("<<gameDirectory>>", StringComparison.Ordinal) && instruction.Action != Instruction.ActionType.Choose)
                {
                    return new PathValidationResult
                    {
                        StatusMessage = "⚠️ Invalid path",
                        DetailedMessage = "Paths must start with <<modDirectory>> or <<gameDirectory>> for security",
                        IsValid = false,
                    };
                }
        ...
        private static string ResolvePlaceholderPath(string path)
        {
            ...
            string resolved = path
                ...
                .Replace("<<kotorDirectory>>/", kotorDir + "/")
                .Replace("<<gameDirectory>>\\", kotorDir + "\\")
                .Replace("<<gameDirectory>>/", kotorDir + "/");
